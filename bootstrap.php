...
if (!file_exists(__DIR__ . '/config.php') && !defined('INSTALLING')) {
    ...
    if (php_sapi_name() === 'cli') {
        ...
        if (file_exists(__DIR__ . '/admin/setup-config.php')) {
            header('Location: /mini/Inventory_Listing-WEB/admin/setup-config.php');
            exit;
        } elseif (file_exists(__DIR__ . '/setup.php')) {
            header('Location: /mini/Inventory_Listing-WEB/setup.php');
            exit;
        } else {
            ...
            if ($r && $row = $r->fetch_assoc()) {
                ...
                if (defined('SYSTEM_VERSION') && version_compare($dbVersion, SYSTEM_VERSION, '<')) {
                    ...
                    if (php_sapi_name() !== 'cli') {
                        $remote = $_SERVER['REMOTE_ADDR'] ?? '';
                        $allowed = ['127.0.0.1', '::1', 'localhost'];
                        if (in_array($remote, $allowed) && file_exists(__DIR__ . '/setup.php')) {
...
// 1. Load theme base functionality first
require_once __DIR__ . '/core/includes/theme.php';

// 2. Determine active theme from settings table (fallback to default)
$active = getSetting('active_theme', 'default');
define('ACTIVE_THEME', $active);
if (!defined('ACTIVE_THEME_PATH')) {
    define('ACTIVE_THEME_PATH', THEMES_PATH . '/' . ACTIVE_THEME);
}

// 3. Load theme-specific functions
...
// برای اطمینان از اینکه تابع get_template از core/includes/theme.php استفاده می‌کند
